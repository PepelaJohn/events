  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-white shadow-md py-3' : 'bg-white min-[1000px]:py-8 py-5'
      }`}
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
    >
      <div className="container mx-auto px-4 min-[1000px]:px-8 flex justify-between items-center">
        <nav className="hidden min-[1000px]:flex space-x-8">
        ...
        <Link href="/" className="mx-auto min-[1000px]:mx-0">
        ...
        {/* Mobile menu button */}
        <button 
          className="min-[1000px]:hidden text-neutral-800"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            className="min-[1000px]:hidden absolute top-full left-0 right-0 bg-white shadow-lg"
